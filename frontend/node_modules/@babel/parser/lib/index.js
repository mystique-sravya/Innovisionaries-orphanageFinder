...
function _objectWithoutProperti'use strict';
...
const FlowErrors = ParseErrorEnum`flow`({
  ...
  EnumInconsistentMemberValues: ({
    enumName
  }) => `Enum \`${enumName}\` has inconsistent member initializers. Either use no initializers'use strict';
...
class ExpressionParser extends LValParser {
  ...
  parseMember(base, startLoc, state, computed, optional) {
    ...
    if (computed) {
      ...
      node.property = this.parse, or consistently use literals (either booleans, numbers, or strings) for all member initializers.`,
  EnumInvalidExplicitType: ({
    invalidEnumType,
    enumName
  }) => `Enum type \`${invalidEnumType}\` is not valid. Use one of \`boolean\`, \`number\`, \`string\`, or \`symbol\` in enum \`${enumName}\`.`,
